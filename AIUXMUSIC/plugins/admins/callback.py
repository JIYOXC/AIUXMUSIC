...
from AIUXMUSIC import YouTube, app
from AIUXMUSIC.core.call import AIUXMUSIC
from AIUXMUSIC.misc import SUDOERS, db
from AIUXMUSIC.utils.database import (is_active_chat,
...
from AIUXMUSIC.utils.formatters import seconds_to_min
from AIUXMUSIC.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await AIUXMUSIC.pause_stream(chat_id)
        ...
        await AIUXMUSIC.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await AIUXMUSIC.stop_stream(chat_id)
        ...
        await AIUXMUSIC.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await AIUXMUSIC.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await AIUXMUSIC.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await AIUXMUSIC.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await AIUXMUSIC.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
